...
def load_data():
    ...
    ######################################################
        ...
        source_DAexamples, target_DAexamples = [], []
        ...
        for line in lines:
            line = line.strip().split()
            path, label = line[0].split('/')[1:], int(line[1])
            source_DAexamples.append((os.path.join(CONFIG.dataset_args['root'], *path), label))
        ...
        test_dataset = DomainAdaptationDataset(source_DAexamples,  target_DAexamples, transform=test_transform)
    ######################################################
